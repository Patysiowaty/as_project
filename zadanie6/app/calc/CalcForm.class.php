...
#require_once $conf->getRootPath() . '/app/calc/Messages.class.php';
class CalcForm
{
    ...
    public function validate(): bool
    {
        ...
        if (!$this->isInRange(1, 10000000, $this->amountOfCredit)) {
            $this->messages->pushError("Amount of credit must be in the range 1 - 10000000!");
            $result = false;
        }

        if (!$this->isInRange(1, 40, $this->creditYears)) {
            $this->messages->pushError("Years of credit must be in the range 1 - 40!");
            $result = false;
        }

        if (!$this->isInRange(0, 200, $this->interestValue)) {
            $this->messages->pushError("Interest value must be in the range 0 - 200!");
            $result = false;
        }
